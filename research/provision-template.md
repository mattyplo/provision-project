# Provision Research Record

> Working template for the first five Provision Project records. The template should evolve when real research exposes gaps in the methodology.

## Record Metadata

- **Provision name:**
- **Working short name:**
- **Record status:** Draft / Research complete / Needs review
- **Last researched:**
- **Researcher(s):**
- **Current Code citation(s):**
- **Related Code citation(s):**
- **Evidence state summary:** Established / Supported / Uncertain / Disputed / Unknown

## 1. Provision Definition and Scope

### What is the provision?

Describe the distinct tax rule or related set of rules being studied.

### Boundaries

State what this record includes and excludes. Do not assume that a single Code section defines the correct unit of analysis.

### Related provisions

Identify definitions, limitations, elections, transition rules, or neighboring provisions that materially affect how this provision operates.

## 2. Current Law

Explain how the provision operates under current law in plain language.

Document, as applicable:

- relevant statutory language and citations;
- important definitions;
- tax treatment or mechanism;
- thresholds, rates, caps, phaseouts, elections, and exceptions;
- interactions with other provisions;
- effective, sunset, or expiration dates;
- material regulations, administrative guidance, or judicial interpretations.

### Current-law evidence state

- **State:** Established / Supported / Uncertain / Disputed / Unknown
- **Basis:**
- **Limitations or unresolved questions:**

## 3. Legal Eligibility

### Who qualifies?

Describe the taxpayers, transactions, assets, income, activities, or circumstances legally eligible for the treatment.

### Who does not qualify?

Record important exclusions or limitations where they help define the provision.

### Eligibility evidence state

- **State:**
- **Basis:**
- **Limitations:**

> Legal eligibility is not evidence that a taxpayer actually uses the provision or ultimately receives its economic benefit.

## 4. Origin and Enactment

### Original enactment or recognizable predecessor

- **Public Law:**
- **Bill:**
- **Enactment date:**
- **Original Code section:**
- **Public Law section:**
- **Statutes at Large citation:**
- **Effective date:**

### What did the original provision do?

Summarize the enacted rule and important differences from current law.

### Historical boundary note

Explain whether the modern provision has a clear single origin or evolved from an earlier policy that requires separate treatment.

### Origin evidence state

- **State:**
- **Basis:**
- **Limitations:**

## 5. Why Was It Created?

Separate the evidence below rather than blending different kinds of historical claims.

### Documented contemporaneous rationale

What did committee reports, conference reports, contemporaneous JCT materials, administration documents, or other enactment-era sources say?

Attribute statements precisely. Do not automatically convert a committee, administration, or individual legislator's explanation into the intent of Congress as a whole.

### Later interpretations

Record later government, academic, historical, or other credible interpretations of the provision's purpose.

### Project inference

If the project draws a conclusion not explicitly stated by the sources, describe the inference, supporting evidence, and reasoning.

### Rationale evidence state

- **State:** Established / Supported / Uncertain / Disputed / Unknown
- **Basis:**
- **Competing explanations:**
- **Limitations:**

## 6. Material Amendment History

Include changes that materially affected eligibility, rates, thresholds, scope, duration, mechanics, or fiscal significance. Minor technical corrections need not be included unless consequential.

| Date | Legislation | Statutory change | Documented rationale | Estimated fiscal effect at the time | Sources |
| --- | --- | --- | --- | --- | --- |
| | | | | | |

### Amendment-history evidence state

- **State:**
- **Known gaps:**

## 7. Observed Use

Keep observed use separate from legal eligibility.

Where data permit, report:

- number of returns or taxpayers using the provision;
- amount deducted, excluded, credited, deferred, or otherwise receiving the tax treatment;
- estimated reduction or deferral in tax liability;
- income distribution;
- business or industry distribution;
- geographic distribution;
- changes in use over time.

For every statistic, preserve the tax year or period, population, unit of observation, source, and relevant limitations.

### What can the data establish?

### What can the data not establish?

### Observed-use evidence state

- **State:**
- **Basis:**
- **Limitations:**

## 8. Direct Tax Benefit

### Who receives the immediate tax treatment?

Describe who experiences the direct reduction, deferral, exclusion, credit, deduction, preferential rate, or other change in tax liability.

### Magnitude and distribution

Report available evidence about the size and distribution of the direct tax benefit. Do not infer economic incidence from this section alone.

### Direct-benefit evidence state

- **State:**
- **Basis:**
- **Limitations:**

## 9. Economic Incidence

### Who ultimately captures the economic value?

Summarize credible empirical evidence concerning who ultimately benefits after relevant behavioral responses, prices, wages, returns to capital, market structure, capitalization, or other effects.

### Evidence

For important studies or analyses, preserve:

- population;
- time period;
- version of the provision studied;
- methodology or identification strategy;
- outcome measured;
- important assumptions;
- robustness, replication, or credible challenges.

### Project assessment

State what the evidence supports and explain any inferential step.

- **Economic incidence:** [finding] / Unknown / Insufficient evidence
- **Evidence state:** Established / Supported / Uncertain / Disputed / Unknown
- **Reasoning:**
- **Important limitations:**
- **What evidence could change this assessment?**

## 10. Distribution

Where evidence permits, describe how use, direct tax benefits, or economic benefits are distributed across relevant populations, such as:

- income groups;
- industries;
- firm sizes;
- geographies;
- household characteristics;
- other provision-specific categories.

Always identify which concept is being distributed: **use**, **direct tax benefit**, or **economic incidence**.

### Distribution evidence state

- **State:**
- **Basis:**
- **Limitations:**

## 11. Estimated Fiscal Effect

Describe the provision's estimated effect on federal revenues using available official estimates where possible.

For each estimate, record:

| Field | Value |
| --- | --- |
| Estimating organization | |
| Publication / estimate identifier | |
| Publication date | |
| Estimate type | Tax expenditure / Revenue estimate / Repeal or reform estimate / Other |
| Years covered | |
| Amount | |
| Baseline or reference law | |
| Behavioral responses incorporated? | |
| Interactions with other provisions | |
| Major methodological limitations | |
| Source | |

### Interpretation

Explain what the estimate measures and, just as importantly, what it does not measure.

Do not describe a tax expenditure estimate as the amount the federal government would necessarily collect if the provision were repealed.

If multiple estimates are compared, consider differences in law, baseline, economic assumptions, data, methodology, and time period before treating changes as changes in the provision's fiscal effect.

### Fiscal-effect evidence state

- **State:**
- **Basis:**
- **Limitations:**

## 12. Administration and Implementation

Where material to understanding the provision, describe:

- Treasury regulations;
- IRS guidance;
- forms and reporting requirements;
- implementation or compliance issues;
- GAO or other evaluations;
- material judicial interpretations.

Distinguish statutory requirements from agency interpretation and implementation.

## 13. Significant Disagreements and Uncertainty

Record credible disagreements rather than forcing consensus.

For each important disagreement, identify:

- the competing conclusions;
- who reached each conclusion;
- differences in data, assumptions, population, time period, or methodology;
- the project's evidence state;
- what remains unresolved.

Stakeholder and advocacy arguments should be attributed rather than presented as independent factual conclusions.

## 14. Source Map

Map important claims to their evidence. A bibliography alone is not sufficient when it obscures which source supports which claim.

| Claim / section supported | Source | Source role | Source status / provenance | Claim relationship | Pinpoint citation | Evidence notes |
| --- | --- | --- | --- | --- | --- | --- |
| | | | | | | |

Suggested source roles include:

- statute;
- regulation;
- administrative guidance;
- judicial interpretation;
- legislative history;
- government estimate;
- government data;
- government analysis;
- empirical research;
- secondary synthesis;
- stakeholder argument;
- journalism.

### Source status and claim relationship

Keep separate what a source **is** from how it is being used for a particular claim.

Where relevant, record source status / provenance such as:

- official or nonofficial;
- primary or secondary; and
- contemporaneous or retrospective.

For each mapped claim, record the source's relationship to that claim where useful:

- **direct support** — the source directly supports the claim;
- **contextual support** — the source helps interpret or contextualize the claim but is not sufficient on its own;
- **lead only** — the source points toward evidence that should be independently retrieved and cited where practical; or
- **inference support** — the source is evidence used by the project to make an explicitly labeled inference.

`Inference support` is not a source role. The underlying source remains, for example, a statute, government dataset, empirical study, or legislative-history source. Confidence or evidence state belongs to the claim or project inference, not to the source itself.

### Source details

For each source used, preserve where applicable:

- title;
- author or issuing institution;
- publication date;
- document or report number;
- stable URL or locator;
- page, section, table, line, or other pinpoint reference;
- date accessed;
- law or proposal version;
- tax year or fiscal years;
- population;
- baseline or estimate type;
- whether the evidence is contemporaneous or retrospective;
- official or nonofficial status, where material;
- primary or secondary status, where material;
- known limitations.

## 15. Project Inferences

List consequential conclusions made by The Provision Project rather than directly stated by a source.

### Inference

- **Conclusion:**
- **Supporting evidence:**
- **Reasoning:**
- **Evidence state:**
- **Limitations:**
- **What would change the assessment?**

Repeat as needed.

## 16. Open Questions

List unresolved research questions, missing documents, unavailable data, and claims that remain Unknown or insufficiently supported.

- 
- 
- 

## 17. Final Source Audit

Before treating the record as initially complete, check for:

- [ ] Provision boundaries are explicit.
- [ ] Current law is supported by current authoritative sources.
- [ ] Eligibility is not confused with observed use.
- [ ] Observed use is not confused with direct tax benefit.
- [ ] Direct tax benefit is not confused with economic incidence.
- [ ] Original enactment and effective date are established or explicitly unresolved.
- [ ] Claims about legislative rationale are precisely attributed.
- [ ] Material amendments are traced to enacted law.
- [ ] Consequential factual and analytical claims are mapped to sources.
- [ ] Project inferences are visibly labeled and carry their own reasoning and evidence state.
- [ ] Source role is kept separate from source status/provenance and claim relationship.
- [ ] Fiscal estimates identify measurement type, period, baseline, and limitations.
- [ ] Tax expenditure estimates are not presented as automatic repeal revenue.
- [ ] Historical estimates are not presented as current.
- [ ] Stakeholder arguments are attributed.
- [ ] Credible disagreements are represented.
- [ ] Missing or conflicting evidence is visible.
- [ ] Primary sources have been used where reasonably obtainable.
- [ ] Citations actually support the claims attached to them.
- [ ] Unknown / insufficient evidence is used where the research does not support a stronger conclusion.

## 18. Research Completion Summary

A record may reach initial research completeness even when some questions remain unresolved.

### Established findings

-

### Supported findings

-

### Uncertain or disputed findings

-

### Unknown / insufficient evidence

-

### Researcher note

Briefly identify the most important limitations of the record and the highest-value next research step, if any.
